...
- **Data**: synthetic `sales_transactions` fact table (1M / 10M / 50M / 200M rows) joined against `dim_customers` (2M rows) and `dim_products` (5K rows), generated by [data/generate_synthetic_data.py](data/generate_synthetic_data.py).
